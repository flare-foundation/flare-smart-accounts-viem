...
async function sendInstruction({
  ...
  console.log("collateral reservation transaction hash:", instructionTransaction.result.hash, "\n");
...
async function logBalances(personalAccountAddress: Address) {
  const personalAccountFxrpBalance = await getFxrpBalance(personalAccountAddress);
  console.log("Personal account FXRP balance:", personalAccountFxrpBalance, "\n");

  const vaults = await getVaults();
  for (const vault of vaults) {
    const vaultBalance = await getVaultBalance(vault.address, personalAccountAddress);
...
async function main() {
  ...
  const collateralReservationAndDepositInstruction = new UpshiftCollateralReservationAndDepositInstruction(
